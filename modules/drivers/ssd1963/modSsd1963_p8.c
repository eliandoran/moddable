...
/*
	SSD1963 display controller driver - 8-bit parallel (i80) interface
	Adapted from ILI9341 8-bit parallel driver
*/
...
#if !defined(MODDEF_SSD1963P8_DC_PIN) || !defined(MODDEF_SSD1963P8_PCLK_PIN)
	#error required pin not defined
#endif
#if !defined(MODDEF_SSD1963P8_DATA0_PIN) || !defined(MODDEF_SSD1963P8_DATA1_PIN) || !defined(MODDEF_SSD1963P8_DATA2_PIN) || !defined(MODDEF_SSD1963P8_DATA3_PIN) || !defined(MODDEF_SSD1963P8_DATA4_PIN) || !defined(MODDEF_SSD1963P8_DATA5_PIN) || !defined(MODDEF_SSD1963P8_DATA6_PIN) || !defined(MODDEF_SSD1963P8_DATA7_PIN)
	#error required data pin not defined
#endif
#ifndef MODDEF_SSD1963P8_HZ
	#define MODDEF_SSD1963P8_HZ (10000000)
#endif
#ifndef MODDEF_SSD1963P8_WIDTH
	#define MODDEF_SSD1963P8_WIDTH 800
#endif
#ifndef MODDEF_SSD1963P8_HEIGHT
	#define MODDEF_SSD1963P8_HEIGHT 480
#endif
#ifndef MODDEF_SSD1963P8_FLIPX
	#define MODDEF_SSD1963P8_FLIPX (false)
#endif
#ifndef MODDEF_SSD1963P8_FLIPY
	#define MODDEF_SSD1963P8_FLIPY (false)
#endif
#ifndef MODDEF_SSD1963P8_BACKLIGHT_ON
	#define MODDEF_SSD1963P8_BACKLIGHT_ON (0)
#endif
#if MODDEF_SSD1963P8_BACKLIGHT_ON
	#define MODDEF_SSD1963P8_BACKLIGHT_OFF (0)
#else
	#define MODDEF_SSD1963P8_BACKLIGHT_OFF (1)
#endif
#ifndef MODDEF_SSD1963P8_COLUMN_OFFSET
	#define MODDEF_SSD1963P8_COLUMN_OFFSET 0
#endif
#ifndef MODDEF_SSD1963P8_ROW_OFFSET
	#define MODDEF_SSD1963P8_ROW_OFFSET 0
#endif
#ifndef MODDEF_SSD1963P8_CMD_BITS
	#define MODDEF_SSD1963P8_CMD_BITS 8
#endif

#define MODDEF_SSD1963P8_OPQUEUE (10)

typedef struct {
	PixelsOutDispatch			dispatch;

#ifdef MODDEF_SSD1963P8_RST_PIN
	modGPIOConfigurationRecord	rst;
#endif
#ifdef MODDEF_SSD1963P8_BACKLIGHT_PIN
	modGPIOConfigurationRecord	backlight;
#endif
#ifdef MODDEF_SSD1963P8_READ_PIN
	modGPIOConfigurationRecord	readEn;
#endif
#ifdef MODDEF_SSD1963P8_TEARINGEFFECT_PIN
...
void xs_ssd1963p8_destructor(void *data)
{
	spiDisplay sd = data;
	if (!data) return;
	...
	gpio_isr_handler_remove(MODDEF_SSD1963P8_TEARINGEFFECT_PIN);
#endif
...
void xs_ssd1963p8(xsMachine *the)
{
	spiDisplay sd;
	...
	esp_lcd_i80_bus_config_t bus_config = {
		.dc_gpio_num = MODDEF_SSD1963P8_DC_PIN,
		.wr_gpio_num = MODDEF_SSD1963P8_PCLK_PIN,
		.data_gpio_nums = {
			MODDEF_SSD1963P8_DATA0_PIN,
			MODDEF_SSD1963P8_DATA1_PIN,
			MODDEF_SSD1963P8_DATA2_PIN,
			MODDEF_SSD1963P8_DATA3_PIN,
			MODDEF_SSD1963P8_DATA4_PIN,
			MODDEF_SSD1963P8_DATA5_PIN,
			MODDEF_SSD1963P8_DATA6_PIN,
...
#ifdef MODDEF_SSD1963P8_CS_PIN
		.cs_gpio_num = MODDEF_SSD1963P8_CS_PIN,
#else
		.cs_gpio_num = -1,	// "-1 will declaim exclusively use of I80 bus"
#endif
		.pclk_hz = MODDEF_SSD1963P8_HZ,
		.trans_queue_depth = MODDEF_SSD1963P8_OPQUEUE,
...
#ifdef MODDEF_SSD1963P8_RST_PIN
	modGPIOInit(&sd->rst, NULL, MODDEF_SSD1963P8_RST_PIN, kModGPIOOutput);
	modGPIOWrite(&sd->rst, 1);
#endif

#ifdef MODDEF_SSD1963P8_READ_PIN
	modGPIOInit(&sd->readEn, NULL, MODDEF_SSD1963P8_READ_PIN, kModGPIOOutput);
	modGPIOWrite(&sd->readEn, 1);		// Read high (inactive)
#endif

#ifdef MODDEF_SSD1963P8_TEARINGEFFECT_PIN
	gpio_pad_select_gpio(MODDEF_SSD1963P8_TEARINGEFFECT_PIN);
	gpio_set_direction(MODDEF_SSD1963P8_TEARINGEFFECT_PIN, GPIO_MODE_INPUT);
	gpio_set_pull_mode(MODDEF_SSD1963P8_TEARINGEFFECT_PIN, GPIO_FLOATING);
	gpio_install_isr_service(0);
	gpio_set_intr_type(MODDEF_SSD1963P8_TEARINGEFFECT_PIN, GPIO_INTR_POSEDGE);
	gpio_isr_handler_add(MODDEF_SSD1963P8_TEARINGEFFECT_PIN, tearingEffectISR, sd);

	sd->startSend = xSemaphoreCreateBinary();
	sd->syncFrames = 1;
#endif

	sd->ops = xQueueCreate(MODDEF_SSD1963P8_OPQUEUE, sizeof(int));
	sd->opZero = 0;
...
#ifdef MODDEF_SSD1963P8_BACKLIGHT_PIN
	modGPIOInit(&sd->backlight, NULL, MODDEF_SSD1963P8_BACKLIGHT_PIN, kModGPIOOutput);
	modGPIOWrite(&sd->backlight, MODDEF_SSD1963P8_BACKLIGHT_OFF);
#endif
}

void xs_ssd1963p8_begin(xsMachine *the)
{
...
void xs_ssd1963p8_send(xsMachine *the)
{
	spiDisplay sd = xsmcGetHostData(xsThis);
	int argc = xsmcArgc;
	const uint8_t *data;
...
void xs_ssd1963p8_end(xsMachine *the)
{
	spiDisplay sd = xsmcGetHostData(xsThis);
	ssd1963End(sd);
}

void xs_ssd1963p8_continue(xsMachine *the)
{
	spiDisplay sd = xsmcGetHostData(xsThis);
	ssd1963Continue(sd);
}

void xs_ssd1963p8_pixelsToBytes(xsMachine *the)
{
	int count = xsmcToInteger(xsArg(0));
	xsmcSetInteger(xsResult, ((count * kCommodettoPixelSize) + 7) >> 3);
}

void xs_ssd1963p8_get_pixelFormat(xsMachine *the)
{
	xsmcSetInteger(xsResult, kCommodettoBitmapFormat);
}

void xs_ssd1963p8_get_width(xsMachine *the)
{
	spiDisplay sd = xsmcGetHostData(xsThis);
	xsmcSetInteger(xsResult, MODDEF_SSD1963P8_WIDTH);
}

void xs_ssd1963p8_get_height(xsMachine *the)
{
	spiDisplay sd = xsmcGetHostData(xsThis);
	xsmcSetInteger(xsResult, MODDEF_SSD1963P8_HEIGHT);
}

void xs_ssd1963p8_get_c_dispatch(xsMachine *the)
{
	xsResult = xsThis;
}

void xs_ssd1963p8_command(xsMachine *the)
{
	spiDisplay sd = xsmcGetHostData(xsThis);
...
void xs_ssd1963p8_get_syncFrames(xsMachine *the)
{
#ifdef MODDEF_SSD1963P8_TEARINGEFFECT_PIN
	spiDisplay sd = xsmcGetHostData(xsThis);
...
void xs_ssd1963p8_set_syncFrames(xsMachine *the)
{
#ifdef MODDEF_SSD1963P8_TEARINGEFFECT_PIN
	spiDisplay sd = xsmcGetHostData(xsThis);
...
void xs_ssd1963p8_close(xsMachine *the)
{
	spiDisplay sd = xsmcGetHostData(xsThis);
	if (!sd) return;
	xs_ssd1963p8_destructor(sd);
...
static const uint8_t gInit[] ICACHE_RODATA_ATTR = {
	...
	0xB0, 7,
		...
		0x00,									// TFT mode
		(MODDEF_SSD1963P8_WIDTH - 1) >> 8,
		(MODDEF_SSD1963P8_WIDTH - 1) & 0xFF,
		(MODDEF_SSD1963P8_HEIGHT - 1) >> 8,
		(MODDEF_SSD1963P8_HEIGHT - 1) & 0xFF,
		0x00,									// RGB sequence
	...
	0xBE, 6, 0x06, 0x80, 0x01, 0xF0, 0x00, 0x00,
	...
	0x2A, 4,
		0x00, 0x00,
		(MODDEF_SSD1963P8_WIDTH - 1) >> 8, (MODDEF_SSD1963P8_WIDTH - 1) & 0xFF,

	// ---- Row address set ----
	0x2B, 4,
		0x00, 0x00,
		(MODDEF_SSD1963P8_HEIGHT - 1) >> 8, (MODDEF_SSD1963P8_HEIGHT - 1) & 0xFF,
...
void ssd1963Begin(void *refcon, CommodettoCoordinate x, CommodettoCoordinate y, CommodettoDimension w, CommodettoDimension h)
{
	...
	xMin = x + MODDEF_SSD1963P8_COLUMN_OFFSET;
	yMin = y + MODDEF_SSD1963P8_ROW_OFFSET;
...
#ifdef MODDEF_SSD1963P8_BACKLIGHT_PIN
		modGPIOWrite(&sd->backlight, MODDEF_SSD1963P8_BACKLIGHT_ON);
#endif
	}
