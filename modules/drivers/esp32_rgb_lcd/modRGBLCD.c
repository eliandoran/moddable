...
void xs_rgblcd(xsMachine *the)
{
	...
	err = esp_lcd_rgb_panel_get_frame_buffer(rd->panel_handle, MODDEF_RGBLCD_NUM_FBS, &rd->fb[0], &rd->fb[1]);
	if (ESP_OK != err) {
		xs_rgblcd_destructor(rd);
		xsmcSetHostData(xsThis, NULL);
 ...
 *
 * Called by Poco/Piu when rendering is complete. Swaps the buffer so the
 * RGB peripheral starts scanning from the freshly rendered one, then waits
 * for VSYNC to confirm the swap before allowing the next frame.
 */
void rgblcdEnd(void *refcon)
{
	...
	/* Wait for VSYNC to confirm the swap (avoids tearing) */
